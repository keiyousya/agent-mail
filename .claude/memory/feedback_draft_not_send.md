...
### 返信は必ず既存スレッドへの返信として作る

相手からのメールに対して返信する場合、**必ず既存のメールスレッドに返信の形で**下書きを作る。
新規メールとして作ってはならない。具体的には以下の3点をすべて満たすこと。

1. `inReplyTo` に返信先メールの Message-ID をセットする
2. `references` にスレッド内の Message-ID を時系列順で入れる
3. 本文末尾に元メールの引用ブロックを付ける（下記フォーマット）
...
2026-09-14にも引用なしで下書きを作り、再び「送信されてきたメールに返信の形で」と差し戻された。
**下書きを保存する前に、本文末尾に引用ブロックがあるか必ず確認する。**
